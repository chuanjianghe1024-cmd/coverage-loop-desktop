...
官方下载内容与 `scripts/aceternity-lock.json` 的 SHA-256 一致时才写入文件。上游更新后需要审查新源码并更新哈希。Bento Grid 通过工作台 CSS 调整布局；Hover Border Gradient 清理未用 import，增加减少动效支持及 effect 依赖。
