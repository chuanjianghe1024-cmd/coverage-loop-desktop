...
export default function App() {
  ...
  return <div className="app-shell"><div className="desktop-titlebar"><div><img src="./app-icon.png" alt=""/><strong>Coverage Loop</strong><span>让每一行，都有迹可循。</span></div></div><aside className="sidebar"><div className="brand"><span className="brand-mark"><img src="./app-icon.png" alt="Coverage Loop 图标"/></span><strong>Coverage<span>Loop</span></strong><span className="version">1.1</span></div><button className="workspace-switch" disabled={busy} onClick={newWorkspace}><div className="workspace-avatar">{data?.project.rootArtifactId.slice(0,1).toUpperCase()||'W'}</div><span><strong>{data?.project.rootArtifactId||'本地工作区'}</strong><small>{data?'Maven 项目':'连接你的第一个项目'}</small></span><Plus size={16}/></button>
    ...
    </motion.div></main>{toast&&<div className="toast" role="status">{toast}<button onClick={()=>setToast('')} aria-label="关闭提示"><X size={15}/></button></div>}</div>;
}
