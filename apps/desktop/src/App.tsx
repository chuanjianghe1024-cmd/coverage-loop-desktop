...
export default function App() {
  ...
  return <div className="app-shell"><Titlebar notify={notify}/><aside className="sidebar"><div className="brand"><span className="brand-mark"><img src="./app-icon.png" alt="Coverage Loop 图标"/></span><strong>Coverage<span>Loop</span></strong><span className="version">{api.version.split('.').slice(0,2).join('.')}</span></div><button className="workspace-switch" disabled={busy} onClick={newWorkspace}><div className="workspace-avatar">{data?.project.rootArtifactId.slice(0,1).toUpperCase()||'W'}</div><span><strong>{data?.project.rootArtifactId||'本地工作区'}</strong><small>{data?'Maven 项目':'连接你的第一个项目'}</small></span><Plus size={16}/></button>
