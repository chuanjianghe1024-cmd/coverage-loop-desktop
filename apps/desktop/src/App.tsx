...
import { Layers3, LayoutDashboard, FolderGit2, History, ArrowUpRight, Plus, Settings2, X, Database, CircleHelp } from 'lucide-react';
...
export default function App() {
  const [page,setPage]=useState<'wizard'|'dashboard'|'history'>('wizard');
  ...
  const open=async(root:string,settings:string)=>{
    const result=await api.request<OpenProject>('/project/open',{rootPomPath:root});
    if(settings)result.config.maven.settingsPath=settings;
  ...
  const showJob=async(id:string)=>{
    ...
    setHistorical(result);setPage('dashboard');
  };
  const newWorkspace=()=>{if(busy)return;setData(null);setConfig(null);setHistorical(null);setWizardKey(n=>n+1);setPage('wizard');};
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark"><Layers3 size={21}/></span><strong>Coverage<span>Loop</span></strong><span className="version">1.0</span></div><button className="workspace-switch" disabled={busy} onClick={newWorkspace}><div className="workspace-avatar">{data?.project.rootArtifactId.slice(0,1).toUpperCase()||'W'}</div><span><strong>{data?.project.rootArtifactId||'本地工作区'}</strong><small>{data?'Maven 项目':'连接你的第一个项目'}</small></span><Plus size={16}/></button>
    <div className="nav-label">WORKSPACE</div><nav><button className={page==='dashboard'?'active':''} disabled={!config} onClick={()=>{setHistorical(null);setPage('dashboard');}}><LayoutDashboard size={17}/>工作看板</button><button className={page==='wizard'?'active':''} disabled={busy} onClick={()=>setPage('wizard')}><FolderGit2 size={17}/>项目向导{config&&<span className="nav-count">{config.selectedModulePaths.length}</span>}</button><button className={page==='history'?'active':''} disabled={!config} onClick={()=>void showHistory().catch(e=>notify(String(e)))}><History size={17}/>运行记录</button></nav>
    ...
    <main className="main"><header className="topbar"><div className="breadcrumb"><span>工作区</span><span>/</span><strong>{data?.project.rootArtifactId||'新建项目'}</strong>{data&&<><span>/</span><span>{page==='wizard'?'配置向导':page==='history'?'运行记录':'工作看板'}</span></>}</div><div className="topbar-right"><span className="local-label"><span className="dot"/>LOCAL</span>{data&&<button className="icon-button" aria-label="编辑项目配置" disabled={busy} onClick={()=>setPage('wizard')}><Settings2 size={17}/></button>}</div></header>
    ...
    <div className="page-content">{page==='wizard'&&<Wizard key={wizardKey} data={data} config={config} setConfig={setConfig} onOpen={open} onFinish={finish} recents={recents} busy={busy} notify={notify}/>}
    ...
    {page==='history'&&<section><div className="page-title"><div><div className="eyebrow">RUN HISTORY</div><h1>每一轮，都有记录。</h1><p>回看任务、覆盖率和测试结果。</p></div><button className="secondary" onClick={()=>void showHistory().catch(e=>notify(String(e)))}>刷新记录</button></div><div className="panel history-panel">{jobs.length?jobs.map(j=><button className="history-row" key={j.id} onClick={()=>void showJob(j.id).catch(e=>notify(String(e)))}><span className="history-icon"><History size={20}/></span><span className="history-info"><strong>{j.name} · {j.mode==='baseline'?'基线验证':j.mode==='loop'?'自动补测':'Agent 检查'}</strong><small>{j.message}</small></span><time>{new Date(j.startedAt).toLocaleString()}</time><Status value={j.status}/><ArrowUpRight size={16}/></button>):<div className="history-empty"><History size={30}/><h3>还没有运行记录</h3><p>第一次任务执行后，会自动出现在这里。</p></div>}</div></section>}
    </div></main>{toast&&<div className="toast" role="status">{toast}<button onClick={()=>setToast('')} aria-label="关闭提示"><X size={15}/></button></div>}</div>;
}
